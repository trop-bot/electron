...
(function () {
  ...
  exports.wrapFsWithAsar = fs => {
    ...
    const { lstat } = fs
    fs.lstat = (pathArgument, options, callback) => {
      const { isAsar, asarPath, filePath } = splitPath(pathArgument)
      if (arguments < 3) {
        callback = options
        options = {}
    ...
    const { statSync } = fs
    fs.statSync = (pathArgument, options, callback) => {
      const { isAsar } = splitPath(pathArgument)
      if (!isAsar) return statSync(pathArgument, options, callback)
    ...
    fs.stat = (pathArgument, options, callback) => {
      ...
      // Do not distinguish links for now.
      process.nextTick(() => fs.lstat(pathArgument, options))
    }
